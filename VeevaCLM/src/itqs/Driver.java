...
public class Driver {
	...
	String ExecuteCompleteTestScriptOnGettingFailedStep;
	...
	{
		this.ExecuteCompleteTestScriptOnGettingFailedStep="";
		this.document=null;
		this.table=null;
		...
		this.TestCaseLevel="Test Case Name,Test Case Description,Run Number,Run Status,Execution Date,Execution Start Time";
		this.TestStepLevel="Step Description,Step Expected,Step Actual,Step Status";
		this.TestScript_Excel_Location=".//Data//Automation_Test_Script_VeevaCRM - Step4.xls";
		this.wordDocumentFileName=".//Result//ExecutionReport.docx";
		this.ScreenShot_File_Location=".//Result//SC.png";
	...
	public static void main(String[] args) {
		...
		SharedVariableClass.InitiateSharedVariableClass();
		LoggerClass.WriteToLog(className, "Driver Variables are Initialized");
		...
		dobj.Test_Case_Record_Count=GenericKeywords.getSQLRecordCount(dobj.TestScript_Excel_Location,dobj.TestCase_SQL_Query);
		if(dobj.Test_Case_Record_Count>0)
		{
			...
			while(dobj.TC_Loop_Count<=dobj.Test_Case_Record_Count)
			{
				...
				testCaseName=dobj.TestCase_Name;
				
				if(dobj.Data_Record_Count>0)
				{
					dobj.TestStep_Record_Count=GenericKeywords.getSQLRecordCount(dobj.TestScript_Excel_Location,dobj.TestStep_SQL_Query);
					while(dobj.Data_Loop_Count<=dobj.Data_Record_Count)
					{
						...
						//******Enter the PASSED execution result in file for ALM update
						//GenericKeywords.createTXTfile("./Result/", dobj.TestCase_ID+"_"+dobj.Data_Run_ID+"_ALM_RESULT_UPDATE.TXT");
						//GenericKeywords.WriteToTXTFile("./Result/", dobj.TestCase_ID+"_"+dobj.Data_Run_ID+"_ALM_RESULT_UPDATE.TXT", "ALM_UPDATE:FAILED");
						GenericKeywords.createTXTfile("./Result/", "ALM_RESULT_UPDATE.TXT");
						GenericKeywords.WriteToTXTFile("./Result/", "ALM_RESULT_UPDATE.TXT", "ALM_UPDATE:FAILED");
						//***********************End*****
						...
						if(dobj.TestStep_Record_Count>0)
						{
							...
							GenericKeywords.writeToWordDocument(dobj.document, dobj.wordDocumentFileName);						
							dobj.ExecuteCompleteTestScriptOnGettingFailedStep=GenericKeywords.getConfigDetails("ExecuteCompleteTestScriptOnGettingFailedStep");
							while(dobj.TestStep_Loop_Count<=dobj.TestStep_Record_Count)//TestStep_Loop_Count<=TestStep_Record_Count
							{
								...
								System.out.println("Execution_Result: " +dobj.dobj_Final_Execution_Result );
								...
								{	
									...
									if (dobj.dobj_Final_Execution_Result==1 || dobj.dobj_Final_Execution_Result==2 ) 
									{
										System.out.println(dobj.TestStep_ID + " : " + dobj.TestStep_Description  + " : Paased");
									...
									else if(dobj.dobj_Final_Execution_Result==0 ||dobj.dobj_Final_Execution_Result==-1 )
									{
										System.out.println(dobj.TestStep_ID + " : " + dobj.TestStep_Description  + " : Failed");
								...
								if(dobj.ExecuteCompleteTestScriptOnGettingFailedStep.equalsIgnoreCase("no") && dobj.step_Fail_Flag==1)
									break;
							...
							if(dobj.TestScript_RunNumber_Fail_Flag==1)
							{
								...
								//******Enter the FAILED execution result in file for ALM update
								//GenericKeywords.createTXTfile("./Result/", dobj.TestCase_ID+"_"+dobj.Data_Run_ID+"_ALM_RESULT_UPDATE.TXT");
								//GenericKeywords.WriteToTXTFile("./Result/", dobj.TestCase_ID+"_"+dobj.Data_Run_ID+"_ALM_RESULT_UPDATE.TXT", "ALM_UPDATE:FAILED");
								//***********************End*****
								dobj.testRunForALM = 1;
							}
							else
							{
								...
								//******Enter the PASSED execution result in file for ALM update
								//GenericKeywords.createTXTfile("./Result/", dobj.TestCase_ID+"_"+dobj.Data_Run_ID+"_ALM_RESULT_UPDATE.TXT");
								//GenericKeywords.WriteToTXTFile("./Result/", dobj.TestCase_ID+"_"+dobj.Data_Run_ID+"_ALM_RESULT_UPDATE.TXT", "ALM_UPDATE:PASSED");
								
								//***********************End*****
								if (dobj.testRunForALM == 1) {
									dobj.testRunForALM = 1;
								}else {
									dobj.testRunForALM = 0;
								}
							}
						...
						SharedVariableClass.clearSharedVariable();
					}
				}
				else
				{
		...
		if (dobj.testRunForALM == 1) {
			...
			GenericKeywords.createTXTfile("./Result/", "ALM_RESULT_UPDATE.TXT");
			GenericKeywords.WriteToTXTFile("./Result/", "ALM_RESULT_UPDATE.TXT", "ALM_UPDATE:PASSED");		
		}
	}
